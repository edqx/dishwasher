...
pub fn main() !void {
    ...
    for (0..100) |_| {
        ...
        var ownedDocument = try dw.parseXmlFull(gpa.allocator(), @embedFile("./gl.xml"));
    ...
    try std.io.getStdOut().writer().print("min: {}ms, max: {}ms, avg: {d:2}ms", .{ min, max, avg });
}
